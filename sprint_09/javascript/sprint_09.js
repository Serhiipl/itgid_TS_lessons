...
class Paragraph_05 {
    ...
    render() {
        const p = document.createElement("p");
        if (this.bold) {
            p.innerHTML = `<b>${this.text.charAt(0).toUpperCase() + this.text.slice(1)}</b>`;
        }
        else {
            p.textContent = this.text.charAt(0).toUpperCase() + this.text.slice(1);
        }
        return p;
    }
}
...
class Cart {
    ...
    total(arr) {
        ...
        for (const item of arr) {
            totalNum += item.price * item.amount;
        }
        return totalNum;
    }
}
