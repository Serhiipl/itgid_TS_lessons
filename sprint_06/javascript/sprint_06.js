...
let f10 = (ar) => {
    let ar101 = [[], []];
    ar.filter((el) => el % 2 == 0 ? ar101[0].push(el) : ar101[1].push(el));
    return ar101;
};
...
let f13 = () => {
    ...
    let out = nArr.map((nRow) => nRow.join(" ")).join("\n");
...
const k16 = [77, 88];
let f16 = () => {
    document.querySelector(".out-16").textContent = String(k16.map((i) => i + 10).reduce((acc, i) => acc + i, 0));
};
...
let k17 = [2, 2, 3, 4];
let f17 = () => {
    let out = k17.reduce((acc, i) => acc + i, 0);
