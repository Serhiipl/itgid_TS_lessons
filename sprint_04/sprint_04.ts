...
function f04(min: number = 0, max: number = 100): number {
...
function f05(): string {
  let randomIndex: number = Math.floor(Math.random() * s5.length);
...
function f07(): void {
  let randomNumber = f04(150, 170);
  document.querySelector(".out-7").textContent = String(randomNumber);
}
...
function f12(pass: string): boolean {
  let bigChart: boolean = false;
  for (let i = 0; i < pass.length; i++) {
    if (pass[i] >= "A" && pass[i] <= "Z") bigChart = true;
  }
  if (pass.length > 8 && bigChart === true) return true;
  return false;
}
...
function f16(a: string): number | boolean {
  return parseInt(a) ? parseInt(a) : false;
}
...
function f17(s: string): string | boolean {
  let domElement: HTMLElement | null = document.querySelector(s);
  //   return !domElement ? false : domElement.textContent.trim();
...
function f19(s1?: string, s2?: string): string {
  ...
  if (s1 === undefined && s2.length > 0) return s2;
  if (s2 === undefined && s1.length > 0) return s1;
